...
class ColorProperty:
    colors = ("red", "green", "cyan", "blue", "black", "magenta", "white", "yellow", "blank")
    fg_colors_dict = {
        "black": "\033[30m", 
        "red": "\033[31m",
        "green": "\033[32m", 
    ...
    def __init__(self, default_fg_color, default_bg_color) -> None:
        if default_fg_color is not None and (default_fg_color not in self.colors or default_fg_color == "blank"):
            raise ValueError(f"default foreground color {default_fg_color} is unsupported")
        if default_bg_color is not None and default_bg_color not in self.colors:
            raise ValueError(f"default background color {default_bg_color} is unsupported")
